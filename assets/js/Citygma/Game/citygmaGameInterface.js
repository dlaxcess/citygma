...
export default class CitygmaGameInterface extends Component {
            ...
            // FIN DU JEU
                ...
                if (this.isInt(this.state.userAdvance)) {
                    ...
                    if(this.state.userAdvance % 0.5 === 0) {
                        ...
                        this.setState({userAdvance: this.state.userAdvance + 0.2, videoPlayerKey: this.state.userAdvance + 0.3, videoUrl: this.state.enigmas[enigmaKey].enigmaVideoHistoryInfo, videoPlaying: false, displayVideo: false, geolocateShow: true, showCompass: true, showEnterGameScreen: false});
                        // Compass Bearing
                        this.activateCompass();
    ...
    handleNearLocationDistance() {
        ...
        // Retour GPS atteinds Boucle
            ...
            console.log('VIDEO 3?');
            const enigmaKey = Math.round(this.state.userAdvance) - 2;
            this.setState({/*videoPlayerKey: this.state.videoPlayerKey, videoUrl: this.state.enigmas[enigmaKey].enigmaVideoHistoryInfo, */videoPlaying: true, displayVideo: true, geolocateShow: false, showCompass: false, userAdvance: this.state.userAdvance + 0.1, currentLat: this.state.enigmas[enigmaKey].enigmaLat, currentLong: this.state.enigmas[enigmaKey].enigmaLong, destinationPrecision: this.state.enigmas[enigmaKey].loopCatchPositionDistance, showEnterGameScreen: false});
        ...
        // Retour enigme DERNIERE BOUCLE & enigme finale
            ...
            if (this.isFloat(this.state.userAdvance)) {
                ...
                if (Math.round((this.state.userAdvance % 0.5)*100)/100 ===0.4) {
                    this.setState({currentEnigmaActiveCompass: true, showEnigma: false, videoPlayerKey: Math.round(this.state.userAdvance), videoUrl: this.state.adventure.videoLastEnigmaFilename, videoPlaying: true, displayVideo: true, geolocateShow: false, showCompass: false, userAdvance: Math.round(this.state.userAdvance), currentLat: this.state.adventure.lastEnigmaLatitude, currentLong: this.state.adventure.lastEnigmaLongitude, destinationPrecision: this.state.adventure.catchPositionDistance, showEnterGameScreen: false});
                    this.storeUserAdvance(Math.round(this.state.userAdvance));
                // Retour enigme FINALE
                }else if (Math.round((this.state.userAdvance % 0.5)*100)/100 ===0.2) {
                    this.setState({/*currentLat: this.state.enigmas[enigmaKey].enigmaLat, currentLong: this.state.enigmas[enigmaKey].enigmaLong, */showEnigma: false, videoPlayerKey: this.state.userAdvance, videoUrl: this.state.adventure.videoFinalSequenceFilename, videoPlaying: true, displayVideo: true, geolocateShow: false, showCompass: false, userAdvance: Math.round(this.state.userAdvance), showEnterGameScreen: false});
                    this.storeUserAdvance(Math.round(this.state.userAdvance));
                }
            }
        }
